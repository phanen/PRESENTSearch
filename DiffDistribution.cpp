  ...
  for (int i = 0; i < 16; i++) {
    for (int j = 0; j < 16; j++) {
      ...
      if (temp != 0 && temp != 16) {
        ...
        DDT_SearchInOrderWithFixedX
            [freq][i][DDT_SearchInOrderWithFixedXLength[freq][i]] = j;
        DDT_SearchInOrderWithFixedXLength[freq][i]++;
        DDT_SearchInOrderWithFixedY
            [freq][j][DDT_SearchInOrderWithFixedYLength[freq][j]] = i;
